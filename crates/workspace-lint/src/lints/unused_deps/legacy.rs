//! The syn-resolver backend of `unused-deps` — the pre-pivot implementation,
//! kept verbatim behind `WL_SEMANTIC_BACKEND=syn` for transitional diffing
//! against the rustc backend (`ir.rs`). Self-contained on purpose (its
//! `Manifest`/`DeclaredDep` are syn-workspace's types, the ir backend's are
//! the fast tier's copies); deleted with syn-workspace in the migration's
//! deletion PR.
