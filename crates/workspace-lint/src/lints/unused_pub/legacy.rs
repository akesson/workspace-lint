//! The pre-pivot syn-resolver `unused-pub` backend, kept verbatim for
//! backend-diffing (`WL_SEMANTIC_BACKEND=syn`) until the migration's deletion
//! PR. See the module docs in [`super`] for the lint's semantics and
//! [`super::ir`] (the default backend) for the rustc-IR implementation.
...
pub(super) fn apply_structural_fix(
...
/// reaches here.
pub(super) fn build_tighten_suggestion(
    item: &Item,
    usage: &Usage,
...
pub(super) fn delete_suggestion(span: &syn_workspace::SourceSpan) -> DeleteOutcome {
...
pub(super) fn is_file_clean_in_git(path: &std::path::Path) -> bool {
